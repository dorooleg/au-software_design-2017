...
class CommandsTest(unittest.TestCase):
    ...
    def test_assignment(self):
        cmd = CommandAssignment([Lexem(LexemType.ASSIGNMENT, 
                                       'qwerty=123', 0, 10)])
    ...
    def test_exit_command(self):
        cmd = CommandExit([Lexem(LexemType.STRING, 'exit', 0, 3)])
        
        assertRaises(ExitException, cmd.run, self.init_input, self.init_env)

    def test_cd_command(self):
        cmd = CommandCd([Lexem(LexemType.STRING, 'cd', 0, 1),
    ...
    def test_cat_command(self):
        cmd = CommandCat([Lexem(LexemType.STRING, 'cat', 0, 2),
    ...
    def test_cat_command_from_stdin(self):
        cmd_1 = CommandPwd([Lexem(LexemType.STRING, 'pwd', 0, 3)])
        cmd_2 = CommandCat([Lexem(LexemType.STRING, 'cat', 5, 7)])
    ...
    def test_wc_cat_nonexistant_file(self):
        cmd = CommandCat([Lexem(LexemType.STRING, 'cat', 0, 3),
        ...
        cmd = CommandCat([Lexem(LexemType.STRING, 'wc', 0, 3),
                          Lexem(LexemType.STRING, 'some_dummy_file_qqqq', 4, 15)])
        cmd_result = cmd.run(self.init_input, self.init_env)
    ...
    def test_echo_command(self):
        cmd = CommandEcho([Lexem(LexemType.STRING, 'echo', 0, 3),
    ...
    def test_wc_command(self):
        cmd_1 = CommandEcho([Lexem(LexemType.STRING, 'echo', 0, 3),
                           Lexem(LexemType.QUOTED_STRING, '"5 c 123    qwe"', 4, 15)])
        cmd_2 = CommandWc([Lexem(LexemType.STRING, 'wc', 16, 18)])
        cmd = CommandChainPipe(cmd_1, cmd_2)
        cmd_result = cmd.run(self.init_input, self.init_env)
    ...
    def test_wc_command_file(self):
        cmd = CommandWc([Lexem(LexemType.STRING, 'wc', 0, 3),
    ...
    def test_pipe_two_cmd(self):
        cmd_1 = CommandEcho([Lexem(LexemType.STRING, 'echo', 0, 4),
                             Lexem(LexemType.ASSIGNMENT, 'yyy=123', 5, 10)])
        cmd_2 = CommandWc([Lexem(LexemType.STRING, 'wc', 14, 15)])
        cmd = CommandChainPipe(cmd_1, cmd_2)
        cmd_result = cmd.run(self.init_input, self.init_env)
    ...
    def test_pipe_three_cmd(self):
        ...
        cmd_2 = CommandWc([Lexem(LexemType.STRING, 'wc', 17, 18)])
        cmd_3 = CommandWc([Lexem(LexemType.STRING, 'wc', 19, 20)])
    ...
    def test_pipe_threeway_ignore_first_cmd(self):
        cmd_1 = CommandEcho([Lexem(LexemType.STRING, 'pwd', 0, 4)])
        cmd_2 = CommandWc([Lexem(LexemType.STRING, 'echo', 5, 8),
                           Lexem(LexemType.STRING, 'qQqQ', 9, 12)])
        cmd_3 = CommandWc([Lexem(LexemType.STRING, 'wc', 13, 15)])
